...
_URL = "https://aihub.or.kr/aihubdata/data/view.do?currMenu=115&topMenu=100&aihubDataSe=realm&dataSetSn=90"
...
PAPER_TRAINING_FPATH_REL = "논문자료 요약/Training/training_논문.zip"
PATENT_TRAINING_FPATH_REL = "논문자료 요약/Training/training_특허전체.zip"

PAPER_VALIDATION_FPATH_REL = "논문자료 요약/Validation/validation_논문.zip"
PATENT_VALIDATION_FPATH_REL = "논문자료 요약/Validation/validation_특허전체.zip"


WHOLE_FPATH_REL = (
    [PAPER_TRAINING_FPATH_REL, PATENT_TRAINING_FPATH_REL],
    [PAPER_VALIDATION_FPATH_REL, PATENT_VALIDATION_FPATH_REL]
)

def generator(fpath_list, yield_entire=True, yield_section=True):
        ...
        with zipfile.ZipFile(fpath, "r") as fp:
            ...
            for fname in flist:
                ...
                for item in data:
                    ...
                    summary_entire = item["summary_entire"]
                    summary_section = item["summary_section"]


                    if yield_entire:
                        for sum_item in summary_entire:
                            yield {
                                "doc_id": doc_id,
                                "summary_type": "entire",
                                "original_text": sum_item["orginal_text"],
                                "summary_text": sum_item["summary_text"],
                            }
                    
                    if yield_section:
                        for sum_item in summary_section:
                            yield {
                                "doc_id": doc_id,
...
class AIHubPaperSummarizationConfig(datasets.BuilderConfig):
    """BuilderConfig for AIHubPaperSummarization."""
    ...
    def __init__(self, file_list: Tuple[List[str]] = WHOLE_FPATH_REL, **kwargs):
        ...
        """
        super(AIHubPaperSummarizationConfig, self).__init__(**kwargs)
        self.training_file_list=file_list[0]
        self.validation_file_list=file_list[1]

        
class AIHubPaperSummarization(datasets.GeneratorBasedBuilder):
    """AIHubPaperSummarization Dataset"""

    BUILDER_CONFIGS = [
        AIHubPaperSummarizationConfig(
            file_list=WHOLE_FPATH_REL,
            name="default",
            version=_VERSION,
            description="AIHub Paper Summarization dataset",
        ),
        AIHubPaperSummarizationConfig(
            file_list=(
                [PAPER_TRAINING_FPATH_REL],
                [PAPER_VALIDATION_FPATH_REL]
            ),
            name="paper_entire",
            version=_VERSION,
            description="AIHub Paper Summarization dataset",
        ...
        AIHubPaperSummarizationConfig(
            file_list=(
                [PATENT_TRAINING_FPATH_REL],
                [PATENT_VALIDATION_FPATH_REL]
            ),
            name="patent_entire",
            version=_VERSION,
            description="AIHub Paper Summarization dataset",
    ...
    BUILDER_CONFIG_CLASS = AIHubPaperSummarizationConfig
    ...
    via 'datasets.load_dataset("aihub_paper_summarization.py", data_dir="<path/to/folder>")'
    """)
    ...
    def _generate_examples(self, fpath_list):
        ...
        yield_entire = True
        if "_section" in self.config.name:
            yield_entire = False
        
        yield_section = True
        if "_entire" in self.config.name:
            yield_section = False

        for idx, item in enumerate(generator(fpath_list, yield_entire, yield_section)):
            yield idx, item
